...
# MergeSensei Deployment Script для мультипроектной среды
# Этот скрипт автоматизирует процесс деплоя приложения на сервер с несколькими проектами
...
GIT_BRANCH="main"
NGINX_SITES_PATH="/etc/nginx/sites-available"  # Путь к конфигурациям Nginx
...
check_existing_projects() {
    ...
    ssh $SERVER_USER@$SERVER_HOST "ls -la $SERVER_BASE_PATH/" || {
        ...
        ssh $SERVER_USER@$SERVER_HOST "sudo mkdir -p $SERVER_BASE_PATH && sudo chown $SERVER_USER:$SERVER_USER $SERVER_BASE_PATH"
    }
...
update_repository() {
    ...
    if ssh $SERVER_USER@$SERVER_HOST "[ -d $SERVER_PATH ]"; then
        ssh $SERVER_USER@$SERVER_HOST "cd $SERVER_PATH && git fetch origin && git reset --hard origin/$GIT_BRANCH"
        log_success "Репозиторий обновлен"
    else
        ssh $SERVER_USER@$SERVER_HOST "git clone -b $GIT_BRANCH $GIT_REPO $SERVER_PATH"
...
server {
    ...
    ssh $SERVER_USER@$SERVER_HOST "echo '$NGINX_CONFIG' | sudo tee $NGINX_SITES_PATH/$PROJECT_NAME"
    
    # Активация сайта
    ssh $SERVER_USER@$SERVER_HOST "sudo ln -sf $NGINX_SITES_PATH/$PROJECT_NAME $NGINX_ENABLED_PATH/"
...
setup_ssl() {
    ...
    ssh $SERVER_USER@$SERVER_HOST "sudo certbot --nginx -d $PROJECT_NAME.$SERVER_HOST --non-interactive --agree-tos --email admin@$SERVER_HOST"
...
main() {
    log_info "Начинаем деплой MergeSensei в мультипроектной среде..."
...
case "${1:-}" in
    ...
    "cleanup")
        ...
        ;;
    *)
        echo "Использование: $0 {deploy|backup|restart|logs|status|rollback|nginx-reload|cleanup}"
        echo ""
        echo "Команды:"
